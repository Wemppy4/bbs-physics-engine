...
import mchorse.bbs_physics.client.forms.PhysicsBodyFormRenderer;
import mchorse.bbs_physics.client.forms.UIPhysicsBodyForm;
...
public class BBSPhysicsClientAddon implements BBSAddonMod
{
    ...
    public void onRegisterClientSettings(RegisterClientSettingsEvent event)
    {
        ...
        /* How a physics body is drawn and how it is edited. Both registries are static maps keyed
         * by the form's class, so an addon's form is as first-class as BBS's own. Done from an
         * event rather than from the Fabric entry point for the same timing reason the form type
         * itself is — see BBSPhysicsAddon. */
        FormUtilsClient.register(PhysicsBodyForm.class, PhysicsBodyFormRenderer::new);
        UIFormEditor.register(PhysicsBodyForm.class, UIPhysicsBodyForm::new);
    }
}
