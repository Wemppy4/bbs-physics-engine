...
public final class ImpulseRadiusHandle extends UIElement implements IUITreeEventListener
{
    ...
    private final RadiusProjection projection = new RadiusProjection();
    ...
    {
        return this.controller.isRecording() || this.controller.panel.isFlying()
            || this.controller.panel.isRunning() || this.controller.getCurrentEntity() == null
            ? null : ImpulseGizmo.selected(this.controller.panel);
    }

    private boolean overMoveHandle()
    {
        var stencil = this.controller.getStencil();
        return stencil.hasPicked() && stencil.getIndex() >= Gizmo.STENCIL_X
            && stencil.getIndex() <= Gizmo.STENCIL_HANDLE_MAX;
    }
    ...
    public boolean click(UIContext context)
    {
        ...
        this.projection.setup(this.controller.panel.getCamera(), this.controller.panel.preview.getViewport(), this.picked);
